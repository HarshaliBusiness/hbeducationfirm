...
router.get('/collegePredictorBBABMS',(req,res)=>{
  res.render('collegePredictorBBABMS');
});


router.get('/topCollegeBbaBMS',(req,res)=>{
  res.render('topCollegeBbaBMS');
});

router.get('/fetchcity', async (req, res) => {
  try {
    const { data, error } = await supabase
            .from('bba_bms_college_info')
            .select('city')
            .not('city', 'is', null)
